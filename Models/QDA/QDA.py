...
def main(load_dataset):
    ...
    #params = evaluate_hyperparameter(QuadraticDiscriminantAnalysis(), X_train, X_test, y_train, y_test)
    ...
    if not os.path.exists(dirname(dirname(abspath(__file__)))
        ...
        os.makedirs(dirname(dirname(abspath(__file__)))
                    + '/Models Results')
    final_results.to_csv(dirname(dirname(abspath(__file__)))+ '/Models Results/Results_QDA.csv')
