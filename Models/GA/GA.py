...
class GeneticAlgorithm:
    ...
    def init_population(self):
        ...
        return np.array(
            [list(np.random.randint(low=0, high=2, size=len(self.feature_list))) for _ in range(self.n_pop)])

    def get_fitness(self, individual):
        columns = [feature for counter, feature in enumerate(self.feature_list) if individual[counter] == 1]
        X_train, X_test, y_train, y_test = train_test_split(self.data[columns], self.data[self.target], test_size=0.2,
                                                            shuffle=False)
...
def main():
    ...
    dataset = pd.read_csv(dirname(dirname(dirname(abspath(__file__)))) + '/Ficheros Outputs/Datos.csv', sep=';',
                          decimal=',')
    ...
    models_for_GA = [LogisticRegression(solver='liblinear'), LinearDiscriminantAnalysis(),
                     QuadraticDiscriminantAnalysis(), RandomForestClassifier(), SVC(),
                     XGBClassifier(use_label_encoder=False,
                                   eval_metric='logloss')]  # , LinearDiscriminantAnalysis(), QuadraticDiscriminantAnalysis(reg_param=0.0), RandomForestClassifier(), XGBClassifier(use_label_encoder=False, eval_metric='logloss')
